...
bool RunJavaScriptByWindowName(char* name, char* js_code) {
    if (!global_app || !name || !js_code) return false;
    ...
    for (l = windows; l != NULL; l = l->next) {
        ...
        // Match the window by its title
        if (g_strcmp0(gtk_window_get_title(win), name) == 0) {
            
            // Retrieve the direct pointer we "tagged" during creation
            WebKitWebView *web_view = (WebKitWebView*)g_object_get_data(G_OBJECT(win), "my_web_view");
...
void* OpenPhysicalWindow(char* name, char* type, char* source, int w, int h, char* fileloc) {
    ...
    GError *error = NULL;
    gboolean loaded = gtk_window_set_icon_from_file(GTK_WINDOW(window), "img/GTKGoBag48.png", &error);
    ...
    // After creating web_view:
    g_object_set_data(G_OBJECT(window), "my_web_view", web_view);
